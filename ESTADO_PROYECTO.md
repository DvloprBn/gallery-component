...
**Todas las fases + Fase 12 completa (12a–12d) + Fase 14 COMPLETA (14a–14d: video de punta a punta) + Fase 15 COMPLETA (15a–15f: layout "libro" + fotolibro de portada en /trabajo + arreglo del ciclado y cuerpo de libro, 2026-09-07).**
...
6. **Fase 15 — Layout "libro" ✅ COMPLETA (15a–15d, 2026-09-07).** El dueño pidió replicar la sección "The Story" de
   ...
   **15f** ✅ *(2026-09-07 — §30)* arreglo del **ciclado scroll↔giro** (era un lazo por el `scrollTo`
   suave; ahora scroll = única fuente de verdad, alineado instantáneo + ventana de silencio en los
   giros del usuario, nunca `flip()` mientras anima —`changeState`—, snap exacto al detenerse) +
   **cuerpo de libro** en CSS decorativo (`.g-book__body` con tapas `rotateY` bajo `perspective`,
   lomo hundido, canto de páginas que crece/encoge con `--book-page`, sombra de suelo). Sigue siendo
   `page-flip`; el libro **three.js/WebGL se pospuso a un proyecto propio** (2ª librería de UI +
   ...
   **Fase 15 completa.** Pendiente transversal (no bloquea): medir **Core Web Vitals** y probar el
   pase de página / scroll fijo / el arreglo del ciclado en un navegador real — previsiblemente en
   el despliegue a `galeria.dvloprbn.dev`. No depende de Stripe ni del despliegue.
