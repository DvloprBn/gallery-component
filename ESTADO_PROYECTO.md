...
6. **Fase 15 — Layout "libro" (en curso).** El dueño pidió replicar la sección "The Story" de
   ...
   suites; `verify-15a.mjs` 9/9 (backend + SSR del frontend). **15b** `page-flip` (StPageFlip) en
   modo HTML bundleado por npm + curl realista con gesto + caída a `grid` (reduced-motion / sin JS
   / fallo de librería). **15c** scroll fijo (`sticky`) que pasa las páginas + teclado + pasada de
   rendimiento. **15d** pulido (portada desde el `theme`, páginas de video, CTA). No depende de
   Stripe ni del despliegue.
