...
**Todas las fases + Fase 12 completa (12a–12d) + Fase 14 COMPLETA (14a–14d: video de punta a punta) + Fase 15 COMPLETA (15a–15g: layout "libro" + fotolibro en /trabajo + libro independiente del scroll con arranque cerrado, 2026-09-07).**

**Fase 15g — libro independiente del scroll + arranque cerrado (2026-09-07).** El dueño probó
la 15f y pidió: el libro **independiente del scroll** (no fijar pantalla, no obligar a hojear
todo — "aún se comporta extraño"), que **arranque como libro cerrado**, y más realismo.
**Revierte** el scroll cinematográfico de la 15c: se eliminó de `BookLayout.tsx` el
`.g-book__track` alto, el `.g-book__stage` sticky, el listener de scroll y toda la sincronía
scroll↔giro (era la fuente real de la rareza). Ahora el libro vive en el flujo normal; se pasa
página solo con **arrastre**, botones **‹ ›**, o **flechas cuando el foco está dentro del
libro** (sin listener global — no secuestra las flechas de la página). **Arranque cerrado**:
estado `open` (arranca `false`); `data-open` en `.g-book__body` oculta las decoraciones del
lado izquierdo hasta abrir; un botón "Abrir libro" cubre la portada (y el botón **›** abre
cuando está cerrado). Realismo: tapa con material oscuro del tema + relieve, oclusión
permanente en la canal al abrir, `flippingTime` 800, `maxShadowOpacity` 0.7. `.pf-showcase`
simplificado (ya no compensa el pin). Modo `static` (SSR) sin cambios. **Verificado**: `tsc` +
`next build` prod OK; backend sin cambios (**121 tests / 17 suites**); `verify-15a.mjs` 14/14
y `verify-showcase.mjs` 20/20 (SSR plano intacto). **NO verificado en navegador**: que ya se
comporte bien sin el scroll, el estado cerrado (puede necesitar ajuste fino de los `%` del
lomo/tapa según dónde ponga `page-flip` la portada), el botón "Abrir", el realismo de la tapa.
Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §31.

**Fase 15f — arreglo del ciclado + forma de libro (2026-09-07).** El dueño revisó el efecto y
reportó "avanza pero se regresa y se cicla". **Causa**: lazo scroll↔giro — al pasar página se
movía el scroll con `behavior:'smooth'`, y ese scroll suave re-disparaba `flip()` antes de que
el anterior terminara. **Cura** (sin cambiar de librería): scroll = única fuente de verdad
mientras está fijo; los giros del usuario (arrastre/botón/tecla) alinean el scroll de forma
**instantánea** con una ventana de silencio; nunca `flip()` mientras `page-flip` anima
(evento `changeState`); *snap* exacto a la página al detenerse el scroll. **Forma de libro**
(CSS decorativo): `.g-book__body` envuelve el host con tapas (`rotateY` bajo `perspective`),
lomo hundido, canto de páginas que crece a la izquierda y encoge a la derecha con
`--book-page`/`--book-count`, y sombra de suelo. Sigue siendo `page-flip`; el libro
three.js/WebGL se deja como **proyecto propio** (sería la 2ª librería de UI, las imágenes
pasarían a texturas perdiendo srcset/lazy/SEO, y no hay navegador aquí para verificar WebGL).
**Verificado**: `tsc` + `next build` prod OK; `verify-15a.mjs` 14/14 y `verify-showcase.mjs`
20/20 (el SSR plano no cambió). **NO verificado en navegador**: que el ciclado esté curado y
cómo se ve el cuerpo de libro. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §30.

**Fase 15e — fotolibro de portada en `/trabajo` (2026-09-07).** Endpoint público nuevo
`GET /showcase`: los 20 elementos publicados más recientes de **todas** las colecciones
públicas, mezclando foto y video (hasta 4 videos, intercalados para que el libro alterne),
**nunca** el original limpio (D9), y descartando video marcado `published` cuyo transcode nunca
terminó. Frontend: `ShowcaseBook.tsx` (nuevo, cliente) monta `BookLayout` en modo "libro" sobre
un álbum sintético + el lightbox; `/trabajo` conserva su `<h1>` compacto y su rejilla de
colecciones, con el libro entre ambos. `.pf-showcase` (CSS nuevo) desplaza el pin de la escena
para que no quede bajo la `.site-header` sticky. **Verificado**: backend `tsc` limpio, **121
tests / 17 suites** (nuevo `media.service.spec.ts`); frontend `tsc` + `next build` prod OK;
`verify-showcase.mjs` **20/20** (contrato de la API + cross-check de que todo está publicado en
una colección pública + SSR de `/trabajo`). **NO verificado en navegador real** (sin herramienta
de browser), como toda la Fase 15: el hojeo, el pin dentro de `/trabajo`, el lightbox y CWV.
Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §29.

**Fase 15d — pulido del layout "libro" (2026-09-07). Cierra la Fase 15.** Sin mecánica nueva:
la portada del fotolibro pinta la primera foto del álbum muy atenuada (`g-book__cover-bg`,
`opacity: 0.16`) detrás del título + un recuento de piezas; la contraportada gana la floritura
"Fin" y reformula el texto hacia la licencia (CTA a `/contacto` intacto). Las copias se montan
sobre el papel con un paspartú (`padding` en `.g-book__leaf` + sombra sutil en la `.g-figure`).
Las páginas de video ya funcionaban (el ▶ de `GalleryImage` + el lightbox con HLS) — solo se
agranda el ▶ en una hoja entera. Accesibilidad: `.g-book` (estático) y `.g-book__stage` (flip)
son `role="region"` con `aria-label` "Fotolibro: {título}" (el de flip incluye "Página N de M").
**Verificado**: `tsc` front limpio; `next build` prod OK (16 rutas); backend sin cambios
(**112 tests / 16 suites**); `verify-15a.mjs` ampliado a **14/14** (cubre el markup nuevo del
SSR). **NO verificado en navegador real** (sin herramienta de browser), como toda la Fase 15:
el pase de página, el scroll fijo, el arrastre, la caída a `grid`/estático y **Core Web Vitals**
siguen sin comprobarse en pantalla — pendiente para cuando haya un entorno con navegador.
Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §28.
...
6. **Fase 15 — Layout "libro" ✅ COMPLETA (15a–15d, 2026-09-07).** El dueño pidió replicar la sección "The Story" de
   nois7.com/world-of-dreams (un fotolibro que se hojea con el scroll). D15 en `PLAN_DESARROLLO.md`
   §4, diseño en `DOCUMENTO_VIVO_ARQUITECTURA.md` §24. Sub-fases: **15a** ✅ *(2026-09-07 — §25)*
   plumbing del valor `book` (6 sitios, sin migración) + `BookLayout.tsx` con portada / pliegos de
   dos imágenes / contraportada (SSR sin JS; móvil = una hoja por CSS); cabecera de la galería
   "slim" en modo libro. De paso se arregló un 500 → 400 en `ImagePipelineService` (imagen con
   header legible pero cuerpo que `libvips` no puede re-codificar). Verificado: 112 tests / 16
   suites; `verify-15a.mjs` 9/9 (backend + SSR del frontend). **15b** ✅ *(2026-09-07 — §26)*
   `page-flip` (StPageFlip v2.0.7, MIT) **bundleado por npm** (1ª librería de UI del proyecto), modo
   HTML → los `<img>` siguen reales; `BookLayout` con tres modos (`static` SSR/sin-JS · `flip` con
   `page-flip` + arrastre + botones ‹ › + toque→lightbox vía botón transparente que la librería sí
   reenvía · `grid` si `prefers-reduced-motion` o si `page-flip` falla). `tsc`/`next build` limpios,
   `page-flip` en los chunks de cliente, `verify-15a.mjs` 9/9 (SSR intacto). **NO verificado en
   navegador** (sin herramienta de browser): el pase de página, el arrastre y las caídas de modo.
   **15c** ✅ *(2026-09-07 — §27)* la sección se queda **fija** (`sticky`, solo escritorio + sin
   `prefers-reduced-motion`) y el progreso de scroll pasa las páginas; sincronía inversa
   página→scroll; teclado (flechas / PageUp-Down); indicador n/total; precarga del pliego
   siguiente; `contain: layout paint`. **NO verificado en navegador** (el pin, el scroll↔flip, el
   teclado, el rendimiento). **15d** ✅ *(2026-09-07 — §28)* pulido: portada con la primera foto
   atenuada + recuento + tipografía/acento del `theme`; contraportada con floritura "Fin" + CTA de
   licencia; paspartú de las copias sobre el papel; ▶ agrandado en las hojas de video (el lightbox
   con HLS ya funcionaba); `role="region"` + `aria-label` con la página en la escena del libro.
   `verify-15a.mjs` 14/14 (markup del SSR). **15e** ✅ *(2026-09-07 — §29)* fotolibro de portada en
   `/trabajo`: endpoint público `GET /showcase` (20 elementos foto+video de todas las colecciones
   públicas, más recientes, mezcla intercalada de hasta 4 videos, nunca el original — D9, descarta
   video sin transcodificar), `ShowcaseBook.tsx` (BookLayout sobre álbum sintético + lightbox),
   `<h1>` compacto conservado, `.pf-showcase` (offset del pin bajo la `.site-header` sticky).
   `media.service.spec.ts` nuevo (10 tests) → **121 tests / 17 suites**; `verify-showcase.mjs` 20/20.
   **15f** ✅ *(2026-09-07 — §30)* arreglo del **ciclado scroll↔giro** + **cuerpo de libro** en CSS
   (`.g-book__body` con tapas `rotateY` bajo `perspective`, lomo, canto de páginas, sombra de suelo).
   **15g** ✅ *(2026-09-07 — §31)* **libro independiente del scroll** — el dueño probó la 15f y pidió
   que no fije la pantalla ni obligue a hojear todo, que arranque cerrado, y más realismo. **Revierte
   la 15c**: fuera el `.g-book__track` alto, el `.g-book__stage` sticky y toda la sincronía
   scroll↔giro (esa era la fuente real del "se comporta extraño"). Ahora el libro vive en el flujo
   normal; se pasa página con arrastre / botones ‹ › / flechas con el foco dentro (sin listener
   global). **Arranque cerrado**: estado `open`, `data-open` en `.g-book__body` oculta las
   decoraciones izquierdas hasta abrir; botón "Abrir libro" sobre la portada. Realismo: tapa con
   material oscuro del tema + relieve, oclusión en la canal al abrir, `flippingTime` 800. Sigue
   siendo `page-flip`; el libro **three.js/WebGL se pospuso a un proyecto propio** (2ª librería de UI
   + imágenes como texturas + sin navegador para verificar).
   **Fase 15 completa.** Pendiente transversal (no bloquea): medir **Core Web Vitals** y probar el
   libro (ya independiente del scroll) en un navegador real — previsiblemente en el despliegue a
   `galeria.dvloprbn.dev`. No depende de Stripe ni del despliegue.
...
| Fecha | Cambio |
|---|---|
| 2026-09-07 | **Fase 15g — Fotolibro: independiente del scroll + arranque cerrado (construida).** El dueño probó la 15f y pidió tres cosas: el libro **independiente del scroll** ("no se debe obligar al usuario a ver todo", "aún se comporta extraño"), que **simule primero un libro cerrado**, y **más realismo**. **Revierte la decisión de la 15c** (sección fija / scroll cinematográfico), ya con el efecto visto en marcha. **Eliminado** de `BookLayout.tsx` (modo `flip`): el `.g-book__track` alto, el `.g-book__stage` con `position:sticky`, el listener `scroll`/`scrollend`, `offsetForPage`/`syncScrollTo`/`scheduleSnap`, `suppressUntil`, `userFlipRef`, el `changeState`→sync y el `keydown` global en `window`. El libro vive en el **flujo normal**; con esto desaparece toda la maquinaria que producía lo "extraño" (era el lazo scroll↔giro — sin scroll de por medio no hay lazo). Se pasa página solo con **arrastre** (nativo de `page-flip`), botones **‹ ›**, y **flechas del teclado cuando el foco está dentro del libro** (el `keydown` se engancha a `rootRef`, el contenedor `.g-book--flip`, y solo reacciona si burbujea desde un hijo enfocado — nunca secuestra las flechas de la página). **Arranque cerrado**: estado `open` (`useState`, arranca `false`; el evento `flip` lo pone a `p > 0`). `.g-book__body` lleva `data-open="false"|"true"`; en cerrado se **ocultan las decoraciones del lado izquierdo** (`.g-book__board--l` + el canto `::before` — no hay "páginas leídas" aún), quedando lomo + tapa/canto derechos como libro shut; al abrir reaparecen con transición de 0.5 s. **Abrir**: `<button class="g-book__open">` cubre la portada (chip "Abrir libro" con pulso sutil, respeta `prefers-reduced-motion`) → `flipRef.current?.flipNext()`; siempre en el DOM (page-flip reubica la hoja), se apaga con `hidden` cuando `open`; el botón **›** hace de "Abrir" también en cerrado; el indicador dice "Cerrado" hasta abrir. **Realismo**: tapa (`.g-book__leaf--cover`) con material oscuro del tema (`color-mix` de `--g-accent`/`--g-fg` sobre casi-negro) + relieve `inset` + veladura diagonal; oclusión **permanente** en la canal cuando el libro está abierto (`.g-book__body[data-open="true"] .g-book__flip::after`); `page-flip`: `flippingTime` 750→**800**, `maxShadowOpacity` 0.65→**0.7**, `usePortrait:true` (móvil = una página, swipe), `swipeDistance:20`, `mobileScrollSupport:true` (el scroll vertical de la página pasa por encima del libro en táctil). El **techo real** sigue siendo WebGL (libro three.js/OGL como proyecto propio, §30.3). **`/trabajo` y `/g/:slug`**: sin cambios de estructura; `.pf-showcase` simplificado a solo espaciado (ya no compensa el pin bajo la `.site-header`). Modo `static` (SSR) **no cambió**. **Verificado**: `tsc` front limpio; `next build` prod OK (16 rutas); backend sin cambios (**121 tests / 17 suites**); `verify-15a.mjs` **14/14** y `verify-showcase.mjs` **20/20** — el SSR (fotolibro plano) no cambió. **NO verificado en navegador real** (sin herramienta de browser): que el libro ya se comporte bien sin el scroll de por medio, cómo se ve el estado cerrado (depende de dónde ponga `page-flip` la portada — puede necesitar ajuste fino de los `%` del lomo/tapa), el botón "Abrir", y el realismo de la tapa. La lógica es ahora mucho más simple (sin sincronía de scroll) pero su comportamiento en pantalla lo confirma el dueño. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §31. |
| 2026-09-07 | **Fase 15f — Fotolibro: arreglo del ciclado + forma de libro (construida).** El dueño revisó el efecto en pantalla: "avanza pero se regresa y luego se cicla y se reestablece", y pidió "dale más forma de libro" conservando el pase de hoja de `page-flip`. **Bug del ciclado** (lógica de la 15c en `BookLayout.tsx`): al pasar página el handler movía el scroll con `window.scrollTo({ behavior: 'smooth' })`; ese scroll suave dispara decenas de eventos `scroll` durante ~400 ms y el listener `onScroll` **volvía a llamar `instance.flip()`** —a veces a la página anterior— antes de que el giro terminara; el guardia `fromScroll.active` (un booleano que se apagaba en el primer evento `flip`) no cubría toda la ventana. Con `Math.round(progress)` oscilando al soltar entre dos páginas → "avanza, regresa, se cicla". **Cura** (sin cambiar de librería): (1) mientras está fija, el scroll es la **única fuente de verdad**, en un sentido; (2) los giros del **usuario** (arrastre/botón/teclado) alinean el scroll a la página nueva de forma **instantánea** (`behavior:'auto'`) con una **ventana de silencio** (`suppressUntil = now+400ms` + `scrollend`) en la que `onScroll` no dispara giros; (3) **nunca** `flip()` mientras `page-flip` anima — se escucha `changeState` (`busy = 'flipping'|'user_fold'`; un `'user_fold'` además marca `userFlipRef` para acompañar el arrastre con el scroll; nuestros `flip()` de scroll solo pasan por `'flipping'` → no se auto-alinean); (4) **snap**: 160 ms tras el último `scroll`, si no anima y no hay silencio, se asienta el scroll **exactamente** sobre `offsetForPage(getCurrentPageIndex())` (umbral 4 px) — nunca a medio camino. `userFlipRef` es un `useRef` de componente; los botones ‹ › y el teclado lo ponen en `true` antes de `flipNext/Prev`. **Forma de libro** (CSS decorativo, `aria-hidden`, detrás de las hojas): `.g-book__body` envuelve el host de `page-flip` con `.g-book__board--l/--r` (tapas 2,6 % más grandes, `rotateY(±1.1deg)` bajo `perspective: 2600px`), `.g-book__spine` (lomo hundido con luz al centro), `.g-book__body::before/::after` (canto de páginas: tacos de hojas apiladas en el borde **exterior** de cada lado — el izquierdo crece y el derecho encoge con `--book-page`/`--book-count`, que `BookLayout` pasa como estilo inline y actualiza en cada `flip`), `.g-book__shadow` (sombra radial de suelo). `.g-book__flip`: `contain: layout paint` → `contain: layout style` (el `paint` recortaría la esquina que se despega); `max-width` movido a `.g-book__body`. `page-flip`: `maxShadowOpacity` 0.5→0.65, `flippingTime` 700→750, `showPageCorners: true`. Las hojas ganan una veladura de papel (dos `linear-gradient` tenues). El modo `static` (SSR) **no cambió**. **three.js/WebGL evaluado y pospuesto**: sería la 2ª librería de UI (pesa en CWV de la portada), las páginas pasarían a ser texturas de `<canvas>` (se pierde `srcset`/lazy/SEO/`<figcaption>` y el pipeline de media deja de aplicar), y no hay navegador en el entorno para verificar una escena WebGL a medida. Queda como **proyecto propio** (sitio artístico inmersivo sobre la obra real del artista), donde el costo de WebGL es el punto. **Verificado**: `tsc` front limpio; `next build` prod OK (16 rutas); backend sin cambios (**121 tests / 17 suites**); `verify-15a.mjs` **14/14** y `verify-showcase.mjs` **20/20** — el SSR (fotolibro plano) no cambió. **NO verificado en navegador real** (sin herramienta de browser): que el ciclado esté curado (scroll↔giro, snap, alineado tras arrastre/botón/tecla) y cómo se ve el cuerpo de libro. La lógica sigue el modelo descrito y los eventos reales de `page-flip` (estados `read`/`user_fold`/`flipping`/`fold_corner`, confirmados leyendo el bundle), pero su comportamiento en pantalla lo confirma el dueño. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §30. |
| 2026-09-07 | **Fase 15e — Fotolibro de portada en `/trabajo` (construida).** A pedido del dueño: montar el fotolibro de la Fase 15 en la cabecera de la página Trabajo, con 20 elementos (foto + video) de **todas** las colecciones públicas — primera vez que `BookLayout` se usa fuera de `/g/:slug`. **Backend — `GET /showcase`** (ruta pública nueva en `MediaController`, bajo el `@Throttle({2400,60s})` de la clase). `MediaService.listShowcase(limit=20)`: consulta `media` con `status='published'` **y** `album.visibility='public'`, `orderBy: created_at desc`, `take: min(limit*3,120)`; **descarta media rota** (sin `variants` y sin `hls_manifest_key` — un video `published` cuyo transcode nunca terminó; en dev había 1 así); **mezcla** hasta `SHOWCASE_MAX_VIDEOS=4` videos (los más nuevos) + resto fotos y los **intercala** (`step = total/(nVideos+1)`) para que el libro alterne en vez de amontonar los videos al inicio. El mapeo fila→`PublicMedia` se extrajo de `getGallery` a un helper `toPublicMedia(row, visibility)`; para el showcase la visibilidad es siempre `'public'` → **nunca** `urls.original` (D9), los videos traen `urls.hls`. Devuelve `PublicMedia[]` directo (no hay álbum). **Frontend**: `lib/api.ts` `fetchShowcase()` (valida `z.array(galleryMediaSchema)`, `safeParse`→`[]`); `components/ShowcaseBook.tsx` (nuevo, cliente) arma una `Gallery` **sintética** (`album.title` = nombre del sitio, `layout:'book'`, `theme:{}`) y monta `<BookLayout onOpen>` + `<Lightbox>` (misma unión que `GalleryView`); `app/trabajo/page.tsx` añade `fetchShowcase()` al `Promise.all`, conserva el `<h1 class="pf-page__title">Trabajo</h1>` + tagline y la rejilla de colecciones, con el libro en medio. **CSS** `.pf-showcase` (nuevo): como en `/trabajo` hay una `.site-header` **sticky**, la escena fijada del libro (`.g-book__stage`, `position:sticky;top:0` desde la 15c) arrancaría tapada → `top: var(--site-header-h,3.25rem)` y `min-height` reducido, solo dentro del mismo `@media` de pin de la 15c. Ningún estilo de la Fase 15 se tocó. **Verificado**: backend `tsc` limpio, **121 tests / 17 suites** (nuevo `media.service.spec.ts`, 10 tests: consulta correcta, nunca `original`, tope 20, cuota de 4 videos, intercalado, HLS en video, clamp de `limit`, descarta video roto, sin videos); frontend `tsc` + `next build` prod OK (16 rutas); `verify-showcase.mjs` **20/20** en vivo (contrato de la API + **cross-check**: cada `mediaId` del showcase aparece en el `/g/:slug` —solo publicadas— de alguna colección pública; SSR de `/trabajo`: `<h1>` compacto, `.pf-showcase`, portada + `role="region"`, 20 `g-book__page` en 10 pliegos, contraportada con CTA, `pf-grid` sigue debajo). **NO verificado en navegador real** (sin herramienta de browser), igual que toda la Fase 15: el hojeo, el pin dentro de `/trabajo` con la `.site-header` sticky, el lightbox y **Core Web Vitals**. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §29. |
| 2026-09-07 | **Fase 15d — Layout "libro": pulido (construida). Fase 15 COMPLETA.** Último tramo, sin mecánica nueva. **Portada** (`BookCover`): recibe `bg` (`media[0].urls.medium ?? small ?? large` — la primera pieza del álbum) → `<img class="g-book__cover-bg">` a `opacity: 0.16` detrás del texto, la "foto de portada impresa"; el texto pasa a `.g-book__cover-inner` (`position: relative; z-index: 1`); recuento `«N piezas»` bajo el título. `.g-book__cover`/`.g-book__back` → `position: relative; overflow: hidden` para recortar el fondo. **Contraportada** (`BookBack`): floritura `<span class="g-book__fin">Fin</span>` (display del `theme`, itálica, acento) + texto reformulado hacia la licencia ("¿Te interesó alguna imagen? Puedo licenciártela…"); CTA a `/contacto` intacto. **Paspartú**: `.g-book__leaf` gana `padding: clamp(0.5rem, 2.4vw, 1.4rem)` + `box-sizing: border-box` y la `.g-figure` interior `border-radius: 2px` + `box-shadow` sutil — la copia se "monta" sobre el papel con margen. **Video en el libro**: no hizo falta código — `GalleryImage` ya pinta `.g-video-badge` (▶ + duración) y tocar la hoja abre el `Lightbox` que ya reproduce HLS (§22); solo se agranda el ▶ (`.g-book__leaf .g-video-badge svg { padding: 0.85rem; transform: scale(1.15) }`). **Accesibilidad**: `.g-book` (modo estático) es `role="region"` + `aria-label="Fotolibro: {título}"`; `.g-book__stage` (modo flip) es `role="region"` + `aria-roledescription="fotolibro"` + `aria-label` con "Página N de M" (se actualiza con `page`; el `.g-book__progress` con `aria-live="polite"` ya anunciaba el cambio desde la 15b). Todo el color/tipografía sale de tokens del `theme` (`--g-accent`, `--pf-display`, `--book-paper`), sin valores fijos nuevos. **Verificado**: `tsc` front limpio; `next build` prod OK (16 rutas); backend sin cambios (**112 tests / 16 suites**); `verify-15a.mjs` ampliado a **14/14** — 4 checks nuevos del SSR (`g-book__count` con "3 piezas", `g-book__cover-bg`, `g-book__cover-inner`, `g-book__fin`, `role="region"` + `aria-label` "Fotolibro:"). **NO verificado en navegador real** (sin herramienta de browser), como toda la Fase 15: el pase de página, el scroll fijo, el arrastre, la caída a `grid`/estático y **Core Web Vitals** siguen sin comprobarse en pantalla — pendiente transversal para cuando haya un entorno con navegador (previsiblemente en el despliegue a `galeria.dvloprbn.dev`). Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §28. |
| 2026-09-07 | **Fase 15c — Layout "libro": scroll fijo + teclado (construida).** El efecto "cinematográfico": en modo `flip`, el libro se envuelve en `<div.g-book__track style="--book-positions:N">` (alto = N × 88vh, la "pista") → `<div.g-book__stage>` (`position: sticky; top:0; 100vh`) → `.g-book__flip`. **El pin solo con `@media (min-width:701px) and (prefers-reduced-motion: no-preference)`** — en móvil la pista mide `auto` y la escena es `static` (nada de scroll-jacking en un teléfono, D15). **Scroll → página**: listener `scroll` pasivo throttleado con `rAF`; si la pista está pineada (`rect.top<=0 && rect.bottom>=innerHeight`) calcula `progress = -rect.top/(rect.height-innerHeight)` y `target = round(progress·(leafCount-1))` → `pageFlip.flip(target)` si cambió. Sale temprano si no está pineada. **Página → scroll** (inversa): en el evento `flip`, si lo disparó el usuario (arrastre/botón/teclado, no nuestro handler — flag `fromScroll`), `window.scrollTo({behavior:'smooth'})` a la posición de la pista de esa página, para que soltar el scroll no "regrese". **Teclado** (con la pista pineada y sin lightbox abierto): `ArrowRight/Down/PageDown` → `flipNext()`, `ArrowLeft/Up/PageUp` → `flipPrev()` (con `preventDefault`). Indicador `n / total` (`aria-live`). **Rendimiento**: el loop de scroll no hace nada si no está pineado; `.g-book__flip { contain: layout paint }`; en cada `flip`, las `<img>` de las hojas `p`,`p+1`,`p+2` que sigan `loading="lazy"` pasan a `eager` (las lejanas siguen perezosas); se sirven los WebP, nunca el original. **Verificado**: `tsc` front limpio; `next build` prod OK; **112 tests / 16 suites** (sin cambios de backend); `verify-15a.mjs` **9/9** — el SSR (fotolibro plano, baseline sin JS) no cambió. **NO verificado en navegador real** (sin herramienta de browser): que la sección se quede fija, que el scroll pase las páginas y las páginas muevan el scroll de vuelta, el teclado, y el rendimiento (CWV). El código implementa el modelo del diseño (§24.2, §24.4) pero su comportamiento en pantalla está sin comprobar. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §27. |
| 2026-09-07 | **Fase 15b — Layout "libro": pase de página con `page-flip` (construida).** Sobre el fotolibro plano de la 15a, monta el pase de página fotorrealista con arrastre (el scroll fijo llega en 15c). **`page-flip` (StPageFlip v2.0.7, MIT, ~44 KB)** bundleada por **npm** (`gallery_frontend/package.json` + lock) — **primera librería de UI del proyecto**, justificada por D15 (un curl de página no es viable en CSS puro). Modo **HTML** (`loadFromHTML`): mantiene `<div>`/`<img>` reales → `srcset`/BlurHash/lazy siguen. No trae `.d.ts` ni CSS en `dist` → declaración mínima en `src/types/page-flip.d.ts` y sus reglas `.stf__*` inlineadas en `globals.css` acotadas bajo `.g-book`. Se importa **dinámicamente** dentro de `useEffect` (`import('page-flip/dist/js/page-flip.module.js')`) — nunca al bundle del servidor ni al render inicial; CSP intacta (no es CDN, no hace red). `BookLayout` pasa a tener **3 modos**: `static` (SSR y lo que queda sin JS — el fotolibro plano de la 15a) · `flip` (cliente, `prefers-reduced-motion` no activo y `page-flip` carga → lista **plana** de hojas `.g-book__leaf`: portada `data-density=hard` → una hoja por medio → hoja en blanco si el nº es impar → contraportada; `showCover`, `size:'stretch'`, `disableFlipByClick:true`, `ResizeObserver`→`update()`) · `grid` (`prefers-reduced-motion` o `page-flip` no carga/falla → reutiliza `GalleryLayout` con `layout:'grid'`). Sin JS se queda en `static` (desviación consciente del "→grid" del diseño; el plano ya es accesible). **Tocar una hoja abre el lightbox**: `page-flip` solo reenvía clics a `<a>`/`<button>`, así que cada hoja lleva un `<button className="g-book__leaf-btn">` transparente a pantalla completa que llama `onOpen(index)` (la imagen va sin `onOpen`); **arrastrar la esquina o los botones ‹ ›** pasan página, **un toque** abre el `Lightbox`. **Verificado**: `tsc` front limpio; `next build` prod OK con `page-flip` en los chunks de cliente (`loadFromHTML` presente); **112 tests / 16 suites** (sin cambios de backend); `verify-15a.mjs` **9/9** — el SSR no cambió, el fotolibro plano (baseline sin JS) está intacto. **NO verificado en navegador real** (sin herramienta de browser, como en las fases de video): el pase de página, el arrastre de la esquina, la "mejora" `static`→`flip`, la caída a `grid` (reduced-motion / fallo de librería), el toque→lightbox vía el botón transparente y los botones ‹ ›. El código sigue la API documentada de `page-flip` y la lógica de fallback es directa, pero lo visual/interactivo está sin comprobar. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §26. |
| 2026-09-07 | **Fase 15a — Layout "libro": plumbing + pliego estático (construida).** Primer tramo de la Fase 15 (el dueño pidió replicar la sección "The Story" de nois7.com/world-of-dreams). Deja el layout `book` funcionando **estático y accesible** (el giro de página y el scroll fijo llegan en 15b/15c). El valor `'book'` se añadió en 6 sitios **sin migración** (`albums.layout` ya es `String @db.VarChar(20)`): `album.dto.ts` `ALBUM_LAYOUTS` (`@IsIn` lo recoge — layout inválido sigue dando 400), `gallery-schema.ts` `z.enum`, `studio-types.ts` `LAYOUTS` + unión `AlbumRow.layout`, `GalleryLayout.tsx` `SIZES_BY_LAYOUT`, `README.md`; el `<select>` de `AlbumSettingsForm` itera `LAYOUTS` → aparece solo. `GalleryView` ramifica: `layout === 'book'` → `<BookLayout>` (nuevo) con cabecera `g-header--slim` (sin `<h1>` — la portada del libro lo lleva). `BookLayout.tsx`: portada (`g-book__cover`, título + descripción + acento del `theme`), **pliegos de dos hojas** (`g-book__spread`) donde cada hoja reutiliza `GalleryImage` (mismo tile: `srcset`, BlurHash, lazy, clic → `Lightbox`), lomo como sombra CSS `::after`, nº impar → última hoja en blanco, folios; contraportada con CTA a `/contacto`. **Móvil (`≤700px`, solo CSS)**: una hoja por pantalla. Es **SSR puro sin JS** — no hay animación que `prefers-reduced-motion` desactive todavía (la caída a `grid` entra en 15b). **Bug propio corregido (500 → 400)**: `ImagePipelineService.process` daba un 500 si un archivo pasaba `sharp().metadata()` (header legible) pero fallaba al re-codificarse (`vips2png: unable to write to target`) — el re-encode estaba fuera del `try/catch` que ya convierte "no es imagen" en 400. Ahora está dentro → 400 "La imagen está dañada o incompleta". Pre-existente desde la Fase 3, no lo introdujo la 15a; test nuevo en `image-pipeline.service.spec.ts`. **Verificado**: `tsc` back+front limpios; **112 tests / 16 suites**; `next build` prod OK; `verify-15a.mjs` **9/9** en vivo (`layout:'flipbook'` → 400; álbum con `layout:'book'` → ok; `GET /g/:slug` `album.layout==='book'` con 3 medios; el **SSR del frontend** (`:3051/g/:slug`) pinta `g-book__cover` con el título, **2 pliegos** para 3 medios con una hoja en blanco, contraportada con CTA a `/contacto`, cabecera `g-header--slim`). **NO verificado en navegador real** (sin herramienta de browser): la apariencia del libro (papel, lomo, folios) no se comprobó visualmente; sí que el markup/clases se sirven y que el layout se selecciona por `album.layout`. Detalle en `DOCUMENTO_VIVO_ARQUITECTURA.md` §25. |
