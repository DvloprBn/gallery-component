...
| **12b. Cotizar** | El gestor responde una solicitud con precio + condiciones + vigencia (`PATCH /license-requests/:id`); correo al solicitante | 12a |
| **12c. Emitir + entregar** | Modelo `licenses` / `delivery_tokens`; al aceptar el solicitante se emite la licencia y una **URL firmada de un solo uso** que entrega el archivo original limpio (con derechos + licenciatario embebidos); registro de licencias emitidas | 12b, fase 11 |
| **12d. Frontend público + pulido** | Botón "Solicitar licencia" en la foto/lightbox con el formulario real; bandeja del gestor unifica contacto + solicitudes; `PRUEBAS_SEGURIDAD.md` | 12a–12c |
