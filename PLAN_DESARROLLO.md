...
| **15. Layout "libro"** *(diseño en `DOCUMENTO_VIVO_ARQUITECTURA.md` §24; sin construir)* | Nuevo layout de álbum **`book`**: un fotolibro que se hojea con el scroll (sección fija + pase de página fotorrealista con `page-flip` en modo HTML, dos imágenes por pliego, portada/contraportada). **Móvil** = una página, gesto para pasar. **`prefers-reduced-motion` / sin JS / fallo de la librería → `grid`**. Sub-fases: **15a** plumbing del layout `book` + pliego estático de dos imágenes + fallback a `grid` (móvil, sin-JS, reduced-motion) · **15b** bundlear `page-flip`, curl realista sobre los pliegos (pasar con gesto/arrastre, sin scroll todavía) · **15c** scroll fijo que pasa las páginas en escritorio + teclado + controles prev/next + pasada de rendimiento (`content-visibility`, precarga del pliego siguiente, higiene de `will-change`) · **15d** pulido (portada desde el `theme`, páginas de video = póster + ▶ → lightbox, CTA de contraportada) | D15, fase 6 (animación) |
