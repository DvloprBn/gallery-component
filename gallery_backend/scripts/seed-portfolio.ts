 ...
 * ficticia «Mara Solís») y cinco colecciones públicas pobladas con fotos
 * reales, subidas por el pipeline de verdad de la API.
 *
 ...
 *
 * Es idempotente: si una colección ya existe (por título) no se recrea, y el
 * `PATCH /site` se limita a fijar los textos. Las fotos de origen son de uso
 * libre; el pipeline del backend las re-codifica y les quita los metadatos.
 */
...
/** Las cinco colecciones y su carpeta(s) de origen. */
const COLLECTIONS: {
  title: string;
  description: string;
  dirs: string[];
...
/** Lista los archivos de imagen de varias carpetas, en orden estable. */
async function listImages(dirs: string[]): Promise<string[]> {
  const all: string[] = [];
  for (const dir of dirs) {
    const full = join(IMAGES_ROOT, dir);
    const entries = (await readdir(full)).filter((f) => IMG_RE.test(f)).sort();
    all.push(...entries.map((f) => join(full, f)));
  }
  return all;
}
...
class Api {
  ...
  async upload(albumId: string, buffer: Buffer, name: string): Promise<void> {
    ...
    if (!res.ok) {
      throw new Error(`Subida "${name}" → ${res.status} ${await res.text()}`);
    }
  }
}
...
async function main(): Promise<void> {
  ...
  // 1. Identidad del sitio (idempotente).
  ...
  for (const col of COLLECTIONS) {
    let album = byTitle.get(col.title);

    if (album) {
      console.log(`» "${col.title}" ya existe (${album.image_count} fotos) — se omite la creación.`);
    } else {
      album = await api.postJson<AlbumRow>('/albums', {
        title: col.title,
      ...
      console.log(`» "${col.title}" creada (slug: ${album.slug}).`);

      const files = spread(await listImages(col.dirs), col.count);
      let i = 0;
      for (const file of files) {
        i += 1;
        const buffer = await prepare(file);
        await api.upload(album.album_id, buffer, `${album.slug}-${i}.jpg`);
        process.stdout.write(`\r   subiendo ${i}/${files.length}…`);
      }
      process.stdout.write('\n');
    }

    // Portada: la primera imagen, si aún no hay uno.
    const fresh = await api.json<AlbumRow>(`/albums/${album.album_id}`);
    if (!fresh.cover_image_id) {
      const images = await api.json<ImageRow[]>(
        `/albums/${album.album_id}/images`,
      );
      if (images.length > 0) {
        await api.patchJson(`/albums/${album.album_id}`, {
          coverImageId: images[0].imageId,
        });
        console.log(`   portada fijada.`);
      }
    }

    if (col.title === 'Calle') {
      heroAlbumSlug = album.slug;
    }
  }

  // 3. Hero de la portada — una foto de "Calle", si no hay uno ya.
  const site = await api.json<{ hero: unknown }>('/site');
  if (!site.hero && heroAlbumSlug) {
    const gallery = await api.json<{ images: ImageRow[] }>(
      `/g/${heroAlbumSlug}`,
    );
  ...
  console.log(
    `\nListo. Abre  ${process.env.FRONTEND_URL ?? 'http://localhost:3051'}/`,
  );
}
